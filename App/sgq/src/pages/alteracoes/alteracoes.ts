                ...
                obras => {
                    switch (alteracao.entidade.toLowerCase()) {
                        ...
                        if(alteracao.tipo.toLowerCase() == "insert") {
                            let index = obras.find(x => x.id == alteracao.obraId).areas.findIndex(x => x.id == alteracao.valor.id);
                            obras.find(x => x.id == alteracao.obraId).areas.splice(index,1);
                            break;
                        } else {
                            obras.find(x => x.id == alteracao.obraId).areas.find(x => x.id == alteracao.valor.id).delete = !alteracao.valor.delete;
                            break;
                        }
                    }
